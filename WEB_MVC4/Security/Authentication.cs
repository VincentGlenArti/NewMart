...
namespace WEB_MVC4.Security
{
    public class Authentication
    {
        ...
        public bool ValidateUserEmail(string email, string password)
        {
            String PassHash = HashPassword(password);
            User User = DataBase.UserSet
                .Where(user => user.Email == email && user.Password == PassHash)
                .Select(user => user)
                .FirstOrDefault();
            return User != null;
        }
        ...
        {
            return DataBase.UserSet.Where(user => user.Email == email).First().Login;
        }
        ...
        public bool Register(RegistrationPageView RegUser)
        {
            ...
            catch (System.Data.Entity.Validation.DbEntityValidationException ex)
            {
                ...
                {
                    foreach (var outval in item.ValidationErrors)
                        System.Diagnostics.Debug.WriteLine(outval.ErrorMessage);
                }
            }
            catch(Exception e)
            {
